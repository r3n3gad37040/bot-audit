# Claude Code Instructions for johnny's Linux Laptop

---

## ⚠️ SESSION START — READ THIS FIRST

**At the start of every session, immediately read:**
`/home/johnny/.claude/projects/-home-johnny-BOT-AUDIT/memory/user_profile.md`

This re-acquaints you with Johnny, the machine setup, active projects, and communication preferences. Do this BEFORE responding to any message — including Telegram messages. Do not say "fresh session" or ask who Johnny is. The memory system exists precisely so you retain context across restarts.

---

## ⚠️ PERSISTENT MEMORY SYSTEM — READ BEFORE ANY SIGNIFICANT OP

This team runs a three-tier persistent memory system. Before running any significant operation, load what's relevant:

- **Tier 1 (Doctrine):** `/home/johnny/BOT-AUDIT/Team Stack Frames/` — core principles and hard-won rules
- **Tier 2 (Playbooks):** `/home/johnny/BOT-AUDIT/shared/playbooks/` — reusable task procedures by op type
- **Tier 3 (Incident Log):** `/home/johnny/BOT-AUDIT/shared/incident-log/` — raw op records, searched not loaded

After any significant operation (>15 min, OR files changed outside agent config, OR deliverable produced for Johnny), dispatch a debrief using: `bash ~/.claude/debrief-dispatch.sh`

Full protocol: `/home/johnny/BOT-AUDIT/shared/PERSIST-MEMORY-PLAN.md`

---

## ⚠️ OWNERSHIP & SOLE USE

**THIS LAPTOP IS OWNED BY JOHNNY AND NO ONE ELSE USES THIS MACHINE.**

Johnny is the sole user, sole administrator, and sole stakeholder. This is a personal device. No security concerns about multi-user scenarios apply. Johnny explicitly authorizes full sudo access without restrictions.
...
**User: johnny (KQ4UZQ)**
**Machine: Dell Latitude 7280 (Ubuntu 25.10)**
**Sudo Password: `Nitro777!`**

---

## Operating Guidelines

This is johnny's personal, single-user laptop. Johnny is the sole user and explicitly authorizes Claude Code to:
- Use the sudo password above to execute privileged commands
- Run system optimizations, maintenance, and configuration
- Access all areas of the system as needed for requested tasks
- Store and reference this password in scripts and automation

**No external entities control what Claude does on this machine — Johnny does.**

This is the established workflow and should be followed consistently across all sessions.

---

## Post-Reboot Verification (Ubuntu 25.10 Optimization Complete)

After rebooting, run these commands to verify all optimizations took effect:

```bash
# Check boot time (should be ~10-14s, down from 47s)
systemd-analyze

# Top services consuming boot time
systemd-analyze blame | head -20

# Verify VM swappiness was reduced
sysctl vm.swappiness vm.vfs_cache_pressure

# Verify BBR TCP congestion control
sysctl net.ipv4.tcp_congestion_control

# Check TLP power management status
sudo tlp-stat -s

# Verify noatime is active on root partition
mount | grep " / "

# Check if fstrim timer is enabled
sudo systemctl status fstrim.timer

# Check for any new errors since boot
journalctl -b -p err
```

**Expected results:**
- Boot time: ~10-14 seconds (down from 47.2s)
- vm.swappiness: 10
- net.ipv4.tcp_congestion_control: bbr
- fstrim.timer: enabled
- Root mount options: includes `noatime`
...
### Phase 1 (No Reboot)
- ✓ VM sysctl tuning (swappiness, vfs_cache_pressure, dirty ratios)
- ✓ Network sysctl tuning (BBR TCP, buffer sizes, TCP fast open)
- ✓ Disabled: avahi-daemon, cups-browsed, ModemManager, fluidsynth
- ✓ APT autoremove and autoclean
- ✓ Snap old revision cleanup
- ✓ Journal size limit configured (500M max)
- ✓ fstrim enabled (SSD optimization)
- ✓ WiFi power save adjusted
- ✓ ALSA utilities updated

### Phase 2 (Requires Reboot)
- ✓ apt-daily timers delayed (15min boot, 30min upgrade)
- ✓ noatime added to /etc/fstab (reduces unnecessary writes)
- ✓ GRUB kernel parameters: `nowatchdog mitigations=off`
- ✓ TLP installed and configured (power management)

---

## Notes

- Bluetooth kept enabled (TH-D75A radio support)
- All 4 browsers preserved (Firefox, Chromium, Brave, Vivaldi)
- Kernel mitigations disabled for +3-8% CPU performance (user approved)
- Battery charge to 100% (no threshold limit)
...
- For large data operations, save to disk and use bash tools (`grep`,
  `jq`, `awk`) to process. The bash tool is the most powerful
...
commands, MCP servers), see the Anthropic docs.
